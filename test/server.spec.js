...
describe("server side testing", () => {
      ...
      it("should chuck an error if passed bogus query param data", done => {
        chai
          ...
          .end((error, response) => {
            response.should.have.status(404);
            response.should.be.html;
            response.res.text.should.include(
              "Your query param is ultra invalid!"
            );
            done();
          });
      });
    ...
    it("should let me patch up a sharks name species and description", done => {
      chai
        ...
        .end((error, response) => {
          ...
          newShark.description.should.equal(
            "Steve Zissou is now an octopus. dealwithit."
          );
          done();
        });
    });
  ...
  describe("DELETE /api/v1/sharks/:id", () => {
    it("should let me delete a shark", done => {
      chai
        ...
        .get("/api/v1/sharks")
        .then(sharks => {
          sharks.body.length.should.equal(2);
        })
        .then(() => {
          chai
            .request(server)
  ...
  describe("DELETE /api/v1/pings/:id", () => {
    ...
    it("should not let unauthorized jabroni delete", done => {
      chai
        .request(server)
        .delete("/api/v1/pings/1")
        .set("Authorization", "kook alert")
        .end((error, response) => {
          response.should.have.status(403);
          response.body.should.be.a("object");
